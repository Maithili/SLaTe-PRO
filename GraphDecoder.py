...
# def sinkhorn(matrix, epsinv=1/0.05, niters=3):
#     matrix = matrix.squeeze(-1)
#     # temp = torch.multiply(matrix, epsinv)
#     # Q = torch.exp(temp)
#     Q = matrix/sum(matrix)
#     _, K, B = Q.shape
#     ones_K = torch.ones([K]).to('cuda') / K
#     ones_B = torch.ones([B]).to('cuda') / B

#     for _ in range(niters):
#         Q = torch.multiply(Q, (ones_K.unsqueeze(0) / torch.sum(Q, dim=1)).unsqueeze(2))
#         Q = torch.multiply(Q, (ones_B.unsqueeze(0) / torch.sum(Q, dim=2)).unsqueeze(1))
...
class GraphDecoderModule(LightningModule):
    ...
    def message_collection_edges(self, edge_influence, topology, context, mask):
        ...
        if self.cfg.edge_importance_normalization:
            topology = sinkhorn(topology)
