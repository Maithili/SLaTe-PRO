...
from ObjectActivityCoembedding import ObjectActivityCoembeddingModule, Latent, LatentVariational, LatentDeterministic, EXTRACAREFUL
...
class MultiModalUserTrackingModule(LightningModule):
    def __init__(self, model_configs, original_model=False):
        ...
        if self.cfg.variational:
            self.variational_mu_var = nn.Sequential(nn.Linear(self.embedding_size, 2*self.embedding_size),
                                                nn.ReLU(),
    ...
    def reset_validation(self):
        ...
        self.result_data = {
            ...
            'relocation_locations_gt':[torch.tensor([]).to('cuda') for _ in range(self.cfg.lookahead_steps + self.cfg.leniency)],
            ...
            'activity_gt':[torch.tensor([]).to('cuda') for _ in range(self.cfg.lookahead_steps + self.cfg.leniency)],
            'relocation_distributions_clarified':{},
            # 'relocation_distributions_with_oracle':{},
            'activity_distributions_clarified':{},
            # 'activity_distributions_with_oracle':{'time':{},'activity':{}, 'variation':{}},
            'num_queries':{},
    ...
    def seq_encoder(self, latents, time_context, seq_type):
        ...
        if seq_type == 'object':
            ...
            if self.cfg.variational:
                encoded_var = self.variational_mu_var(encoded)
                encoded_latent = LatentVariational(encoded_var)
            else:
    ...
    def predict(self, latents, time_context, latents_expected=None):
        ...
        # assert pred_latents.size()[1] > 0, (pred_latents.size(), latents.size(), time_context.size() )
    ...
    def forward(self, batch):
        ...
        ## Mai::Handle single and multi activities???
        ...
        if not self.original_model:
            ...
            # assert (activity_latents[latent_mask] == 0).all(), 'Masked activities have non-zero latents!'
            ...
            # fancy_margin = max(min(math.cos(2*math.pi/self.cfg.n_activities),1),0)                                                                            
            ...
            # latents =  (graph_latents + activity_latents).sample()
            ...
            #             'object_masked' : torch.mean(torch.linalg.norm(graph_latents, dim=-1)),
            #             'activity_masked' : torch.mean(torch.linalg.norm(activity_latents, dim=-1))}
            # latent_magn['object_wrt_activity'] = latent_magn['object_masked']/latent_magn['activity_masked']
    ...
    def query_in_the_loop(self, 
                          ...
                          pred_edges, 
                          expected_activities, 
                          activity_embedding_matrix, 
                          activity_id_seq,
                          query_asked):
            ## Ask queries when not confident in activity predictions
            ...
            if not self.original_model:
                ...
                def usefuleness_metric(pred_edges_original, pred_edges_new):
                    if self.cfg.query_usefulness_metric == 'expected_changes':
                        ...
                        def entropy(pred_edges):
                            return -(pred_edges*(pred_edges+1e-8).log()).sum(-1).sum(-1)
                        return torch.clip(entropy(pred_edges_original) - entropy(pred_edges_new), min=0)
                
                def probs_and_pred_for_mask(mask, mask_type=None, look_ahead=False):
                    _activities_probs = None
                    _latents = deepcopy(latents_forward)
                    if mask is None:
                        latent_in = _latents + time_context[:,step+1:pred_seq_len+step+1] if self.cfg.addtnl_time_context else _latents
                        _pred_new, _, _ = self.object_activity_coembedding_module.decode_graph(latents=latent_in, 
                                                                                                input_nodes=input_nodes_forward, 
                                                                                                input_edges=input_edges_forward, 
                                                                                                dynamic_edges_mask=graph_dyn_edges[:,1+step:pred_seq_len+1+step,:,:]
                                                                                                )
                    elif mask_type == 'activity':
                        _activities = pred_activity*mask
                        _activities_probs = deepcopy(_activities.sum(-1))
                        _activities = _activities/_activities_probs.unsqueeze(-1)
                        _latent_correction = self.object_activity_coembedding_module.activity_encoder(_activities @ activity_embedding_matrix.T)
                        _latents = (1 - self.cfg.query_trust) * latents_forward.sample() + self.cfg.query_trust * _latent_correction.sample()
                        latent_in = _latents + time_context[:,step+1:pred_seq_len+step+1] if self.cfg.addtnl_time_context else _latents
                        _pred_new, _, _ = self.object_activity_coembedding_module.decode_graph(latents=latent_in, 
                                                                                                input_nodes=input_nodes_forward, 
                                                                                                input_edges=input_edges_forward, 
                        ...
                        raise NotImplementedError("Object mask not implemented yet")
                
                    else:
                        raise ArgumentError("Unknown mask type: {}".format(mask_type))

                    _pred_new_far = deepcopy(_pred_new)
                    _lookahead_latent = deepcopy(_latents)
                    if look_ahead:
                        for step_hal in range(step+1, num_steps):
                            _lookahead_latent, _ = self.predict(_lookahead_latent, time_context[:,step_hal:pred_seq_len+step_hal])
                            latent_in = _lookahead_latent + time_context[:,step_hal+1:pred_seq_len+step_hal+1] if self.cfg.addtnl_time_context else _lookahead_latent
                            _pred_new_far, _, _ = self.object_activity_coembedding_module.decode_graph(latents=latent_in,
                                                                                    input_nodes=input_nodes_forward, 
                                                                                    input_edges=_pred_new_far, 
                                                                                    dynamic_edges_mask=graph_dyn_edges[:,1+step_hal:pred_seq_len+1+step_hal,:,:])
                        
                    return _activities_probs, _pred_new, _pred_new_far, _latents
                
                def oracle(query_activities):
                    if self.cfg.precise_oracle:
                        oracle_positive = (expected_activities == query_activities)
                    else:
                        oracle_positive = torch.zeros_like(expected_activities)
                        for ostep in range(num_steps):
                            oracle_expected_activities = activity_id_seq[:,1+ostep:pred_seq_len+1+ostep]
                            oracle_positive = torch.bitwise_or((oracle_expected_activities == query_activities), oracle_positive).clone()
                    return oracle_positive, torch.bitwise_not(oracle_positive)

                # ## Not Looking farther ahead
                activities_if_pred_probs, pred_edges_if_pred, _, latent_if_pred = probs_and_pred_for_mask(F.one_hot(pred_activities, num_classes=self.cfg.n_activities), mask_type='activity')
                activities_if_not_pred_probs, pred_edges_if_not_pred, _, latent_if_not_pred = probs_and_pred_for_mask(1-F.one_hot(pred_activities, num_classes=self.cfg.n_activities), mask_type='activity')
                usefulness_of_query_activity = activities_if_pred_probs * usefuleness_metric(pred_edges, pred_edges_if_pred) + \
                                  activities_if_not_pred_probs * usefuleness_metric(pred_edges, pred_edges_if_not_pred)
                
                # ## Looking farther ahead
                # _, _, far_original, _ = probs_and_pred_for_mask(None, look_ahead=True)
                # activities_if_pred_probs, pred_edges_if_pred, far_if_pred, latent_if_pred = probs_and_pred_for_mask(F.one_hot(pred_activities, num_classes=self.cfg.n_activities), mask_type='activity', look_ahead=True)
                # activities_if_not_pred_probs, pred_edges_if_not_pred, far_if_not_pred, latent_if_not_pred = probs_and_pred_for_mask(1-F.one_hot(pred_activities, num_classes=self.cfg.n_activities), mask_type='activity', look_ahead=True)
                # usefulness_of_query_activity = activities_if_pred_probs * usefuleness_metric(far_original, far_if_pred) + \
                #                     activities_if_not_pred_probs * usefuleness_metric(far_original, far_if_not_pred)
                oracle_positive_activity, oracle_negative_activity = oracle(pred_activities)

                correction_mask = torch.bitwise_and(usefulness_of_query_activity > self.cfg.query_usefullness_thresh, torch.bitwise_not(query_asked))
                query_asked = torch.bitwise_or(query_asked, correction_mask)

                self.results['queries'][step]['num_predictions'] += int(torch.numel(correction_mask))
                self.results['queries'][step]['num_asked'] += int(correction_mask.sum())
                pred_edges[torch.bitwise_and(correction_mask, oracle_positive_activity)] = pred_edges_if_pred[torch.bitwise_and(correction_mask, oracle_positive_activity)]
                pred_edges[torch.bitwise_and(correction_mask, oracle_negative_activity)] = pred_edges_if_not_pred[torch.bitwise_and(correction_mask, oracle_negative_activity)]
                latents_forward = latents_forward.sample()
                latents_forward[torch.bitwise_and(correction_mask, oracle_positive_activity)] = latent_if_pred[torch.bitwise_and(correction_mask, oracle_positive_activity)]
                latents_forward[torch.bitwise_and(correction_mask, oracle_negative_activity)] = latent_if_not_pred[torch.bitwise_and(correction_mask, oracle_negative_activity)]
                latents_forward = LatentDeterministic(latents_forward, self.cfg.learn_latent_magnitude)
                

                return pred_edges, latents_forward, query_asked

    ## time_context[:,:pred_seq_len+num_steps]
    def rollout_onestep_with_masks(self, latent_series, time_context_series, num_steps, graph_seq_edges, input_nodes_forward, time_context, graph_dyn_edges, graph_masks_old, activity_masks_old, query_time, query_type, query_mask, reference_activity, activity_embedding_matrix):
        ...
        def correct_activity(pred_activity, activity_mask):
            ...
            # assert torch.allclose(_corrected_pred_activity*(1-activity_mask), torch.tensor([0.]).to('cuda')), "Activity mask is not working!!"
        ...
        activity_mask_flag = False
        if activity_masks_old is not None:
            activity_mask = activity_masks_old[0]
            act_query_applied = torch.zeros((activity_mask.size()[0], activity_mask.size()[1])).to(bool).to('cuda')
            activity_mask_flag = True
            query_time_act = activity_masks_old[1]
            if reference_activity[0] == activity_mask.argmax(-1):
                query_time_act = num_steps - 1
            ...
            ### TODO: Get the 'continue doing X' i.e. query same as reference to be applied for the whole window!
        ...
        for step in range(num_steps):
            ...
            if not self.original_model:
                ## TODO: Check if the latents predicted before and the unused ones at the next prediction are the same!
                latent_series, _ = self.predict(latent_series, time_context_series[:,step:-num_steps+step])
                latents_forward = latent_series[:,-1:,:]
                # latent_series = torch.cat([latent_series, latents_forward.sample()], dim=1)
            else:
                latents_forward = LatentDeterministic(time_context[:,step+1:pred_seq_len+step+1], self.cfg.learn_latent_magnitude)
            
            graph_mask_flag = graph_masks_old[step] is not None or query_type == 'graph'
            assert not torch.isnan(latents_forward.sample()).any()

            if activity_mask_flag and graph_mask_flag:
                for correction_iteration in range(1):
                    _latent_correction = LatentDeterministic(torch.zeros_like(latents_forward.sample()).to('cuda'), learn_magnitude=self.cfg.learn_latent_magnitude)
                    ## Activity Prediction
                    ...
                    if activity_mask is not None and query_time_act == step:
                        ...
                        _latent_correction += _latent_correction_act
                    ...
                    if graph_masks_old[step] is not None:
                        _edges = ((pred_edges+1e-8)*graph_masks_old[step])
                    ...
                    _latent_correction_graph = self.object_activity_coembedding_module.graph_encoder(nodes=input_nodes_forward, edges=(input_edges_forward,pred_edges), time_context=time_context[:,step+1:pred_seq_len+step+1])
                    _latent_correction += _latent_correction_graph
                    
                    ## Latent Correction
                    if isinstance(latents_forward, Latent): latents_forward = latents_forward.sample()
                    if isinstance(_latent_correction, Latent): _latent_correction = _latent_correction.sample()
                    _latents = deepcopy((1 - self.cfg.query_trust) * latents_forward + 
                                        self.cfg.query_trust * _latent_correction)
                    latents_forward = LatentDeterministic(_latents, learn_magnitude=self.cfg.learn_latent_magnitude)

            elif activity_mask_flag:
                ## Activity Prediction
                latent_in = latents_forward + time_context[:,step+1:pred_seq_len+step+1] if self.cfg.addtnl_time_context else latents_forward
                pred_activity, _, _ = self.object_activity_coembedding_module.decode_activity(latents=latent_in)
                ...
                if graph_masks_old[step] is not None:
                    ...
                    _edges_sum = _edges.sum(-1).clone().detach()
                    pred_edges = _edges.clone().detach()/(_edges_sum.unsqueeze(-1))
                    # assert torch.allclose(pred_edges*(1-graph_masks_old[step]), torch.tensor([0.]).to('cuda')), "Graph mask is not working!!"
                if query_type == 'graph':
                    _edges = ((pred_edges+1e-8)*query_mask)
                    _edges_sum = _edges.sum(-1).clone().detach()
                    if query_time is None:
            ...
            if query_type is None:
                ...
                # activity_best_step[torch.bitwise_and(activity_best_step == num_steps-1, F.one_hot(pred_activities, num_classes=self.cfg.n_activities).bool())] = step
        ...
        if query_type == 'graph':
            if query_time is not None:
                changes_that_must_happen = query_mask.sum(-1).argmin(-1)
                changes_actually_predicted = changes_pred.squeeze(1)
                assert (changes_actually_predicted[torch.arange(changes_that_must_happen.size()[0]), changes_that_must_happen.view(-1)]).all(), \
                    f"Not all enforced changes are predicted!! Got only {(changes_actually_predicted[torch.arange((changes_that_must_happen).size()[0]), changes_that_must_happen.view(-1)]).sum()}/{torch.numel(changes_that_must_happen)}"
            else:
                changes_that_must_not_happen = query_mask.sum(-1).argmin(-1)
                changes_actually_predicted = changes_pred.squeeze(1)
                assert not (changes_actually_predicted[torch.arange((changes_that_must_not_happen).size()[0]), changes_that_must_not_happen.view(-1)]).any(), \
                    f"Some enforced static objects are predicted to move!! Moving {(changes_actually_predicted[torch.arange((changes_that_must_not_happen).size()[0]), changes_that_must_not_happen.view(-1)]).sum()}/{torch.numel(changes_that_must_not_happen)}"

        # if query_type is not None:
        mask = (activity_probs == -1)
        activity_probs[mask] = pred_activity[mask]

        # if activity_mask_flag and activity_mask is not None:
        #     assert (activity_probs.argmax(-1) == activity_mask.argmax(-1)).all(), f"Activity mask not working!! {activity_probs.argmax(-1)} vs {activity_mask.argmax(-1)}"
    ...
    def evaluate_prediction(self, batch, num_steps=1):
        ...
        pred_seq_len = sequence_len-num_steps-self.cfg.leniency
        ...
        # relocations_best_prob_anydest = torch.zeros((batch_size_act, pred_seq_len, self.cfg.n_nodes)).to('cuda')
        # relocations_best_step_anydest = 100 * torch.ones((batch_size_act, pred_seq_len, self.cfg.n_nodes)).to('cuda')
        # relocations_probs_anydest = torch.zeros((batch_size_act, pred_seq_len, self.cfg.n_nodes, self.cfg.n_nodes)).to('cuda')
        ...
        highest_act_change_prob, highest_act_change_step = 100, -1

        query_asked = torch.zeros((batch_size_act, pred_seq_len)).to('cuda').to(bool)
        for step in range(num_steps):
            if not self.original_model:
            ...
            if self.cfg.query_opportunity == 'inline':
                pred_edges, latents_forward, query_asked = self.query_in_the_loop(input_nodes_forward, input_edges_forward, graph_dyn_edges, latents_forward, time_context, step, num_steps, pred_seq_len, pred_activity, pred_edges, expected_activities, activity_embedding_matrix, activity_id_seq, query_asked)
            ...
            # confidence_in_relocation_anydest = (pred_edges * (1 - F.one_hot(reference_edges, num_classes = pred_edges.size()[-1]))).sum(-1)
            # better_confidence_anydest = confidence_in_relocation_anydest > relocations_best_prob_anydest
            # relocations_best_prob_anydest[better_confidence_anydest] = confidence_in_relocation_anydest[better_confidence_anydest]
            # relocations_best_prob_anydest[better_confidence_anydest] = step
            # # relocations_best_prob_anydest[new_changes_pred] = step
            # relocations_probs_anydest[better_confidence_anydest] = deepcopy(pred_edges[better_confidence_anydest])
            ...
            # assert ((pred_edges.argmax(-1) == expected_objects)[used_mask]).sum() == torch.bitwise_and(correct, used_mask).sum()
            # assert ((new_correct)[used_mask]).sum() == torch.bitwise_and(correct, used_mask).sum()
            ...
            # input_edges_forward[graph_dyn_edges[:,1+step:pred_seq_len+1+step,:,:].to(bool)] = pred_edges[graph_dyn_edges[:,1+step:pred_seq_len+1+step,:,:].to(bool)].clone().detach()
            # input_edges_forward = F.one_hot(pred_edges.argmax(-1), num_classes=pred_edges.size()[-1]).float()
            # if step < num_steps-1:
            #     input_edges_forward = deepcopy(graph_seq_edges[:,1+step+1:-num_steps+step+1,:,:])
            
            if len(self.cfg.query_types) > 0 and self.cfg.query_opportunity == 'last' and step == self.result_data['query_step']:
                for query_type in self.cfg.query_types:
                    ...
                    data_for_queries = {'batch': batch, 'num_steps': self.result_data['query_step']+1,
                                    ...
                                    'snapshot_pred': deepcopy(snapshot_pred.detach())}

                    self.evaluate_queries(adict(data_for_queries))
        
        for addtnl_step in range(self.cfg.leniency):
            step = num_steps + addtnl_step
            expected_objects = graph_seq_edges[:,2+step:pred_seq_len+2+step,:,:].argmax(-1)
            expected_activities = activity_id_seq[:,1+step:pred_seq_len+1+step]
            new_changes_gt = deepcopy(torch.bitwise_and(expected_objects != reference_edges, torch.bitwise_not(changes_gt)))
            dest_gt[new_changes_gt] = deepcopy(expected_objects[new_changes_gt])
    ...
    def evaluate_queries(self, data_in):
        assert self.cfg.no_query_carryover, f"STOP CARRYING OVER QUERIES!!!"

        # print(f"Evaluating queries for {self.cfg.query_type} query type")
        ...
        # reference_edges_all = deepcopy(data_in.batch.get('edges')[:,1:pred_seq_len+1,:,:].argmax(-1).clone().detach())
        # reference_edges_backup = deepcopy(data_in.batch.get('edges')[:,1:pred_seq_len+1,:,:].argmax(-1).clone().detach())
        ...
        for curr_step in range(pred_seq_len):
            graph_masks = [None for _ in range(data_in.num_steps)]
            activity_masks = None
            ...
            if data_in.initial_latents_forward is not None:
                latents_forward = deepcopy(data_in.initial_latents_forward[:,curr_step:curr_step+1,:])
                latent_series = deepcopy(data_in.initial_latents_forward[:,:curr_step+1,:])
            else:
                latents_forward = None
                latent_series = None 

            if not self.cfg.no_query_carryover and curr_step > 0:
                new_obj_queries_buffer = []
                for obj_q in obj_queries_buffer:
                    if obj_q['steps_valid'] <= 0: continue
                    query_obj = obj_q['query'][0].detach().item()
                    if obj_q['query'][1] and data_in.changes_gt[:,curr_step-1:curr_step,query_obj]: continue
                    new_obj_queries_buffer.append(obj_q)
                    object_mask_single_row = soften_mask(F.one_hot(torch.tensor([query_obj]), num_classes=self.cfg.n_nodes).float())
                    if obj_q['query'][1]:
                        query_time_mask = (object_mask_single_row * data_in.relocations_best_step[:,curr_step:curr_step+1,:].repeat(object_mask_single_row.size()[0],1,1)).sum(-1)
                        object_mask = (object_mask_single_row.unsqueeze(0).unsqueeze(-1)*(1-reference_edge_mask))+(1-object_mask_single_row).unsqueeze(0).unsqueeze(-1)
                        if graph_masks[query_time_mask.to(int).item()] == None: graph_masks[query_time_mask.to(int).item()] = object_mask
                        else: graph_masks[query_time_mask.to(int).item()] *= object_mask
                    else:
                        object_mask = (object_mask_single_row.unsqueeze(0).unsqueeze(-1)*reference_edge_mask)+((1-object_mask_single_row).unsqueeze(0).unsqueeze(-1))
                        for t in range(obj_q['steps_valid']):
                            if graph_masks[t] == None: graph_masks[t] = object_mask
                            else: graph_masks[t] *= object_mask
                    obj_q['steps_valid'] -= 1
                # print(f"Carrying forward Object {len(new_obj_queries_buffer)}, ", end='')
                obj_queries_buffer = new_obj_queries_buffer
                num_carried_obj_queries += len(obj_queries_buffer)
                if act_queries_buffer is not None and act_queries_buffer['steps_valid'] > 0 and act_queries_buffer['query'].item() != visible_activity_id_seq[:,curr_step-1].item():
                    act_queries_buffer['steps_valid'] -= 1
                    activity_masks = (F.one_hot(act_queries_buffer['query'].unsqueeze(1).to(int).to('cuda'), num_classes=self.cfg.n_activities).float().clone().detach())
                    activity_masks = (activity_masks, min(activity_best_step, act_queries_buffer['steps_valid']))
                    num_carried_act_queries += 1
                    queries_act_viz_carryover[curr_step] = act_queries_buffer['query']
                    # print("Activity 1")
                else:
                    act_queries_buffer = None
                    activity_masks = None
                ...
                if len(obj_queries_buffer) > 0 or act_queries_buffer is not None:
                    ## no_query
                    relocatn_probs_noquery, _, pred_activity_noquery, changes_at_step_wo_query, activity_best_step = self.rollout_onestep_with_masks(
                                                    latent_series=LatentDeterministic(deepcopy(latent_series.sample()), learn_magnitude=latent_series.learn_magnitude) if latent_series is not None else None,
                                                    time_context_series=deepcopy(time_context[:,:curr_step+1+data_in.num_steps,:]),
                                                    num_steps=data_in.num_steps, 
                                                    graph_seq_edges=deepcopy(graph_seq_edges[:,curr_step+1:curr_step+data_in.num_steps+2]),
                                                    input_nodes_forward=deepcopy(graph_seq_nodes[:,curr_step+1:2+curr_step,:,:]), 
                                                    time_context=deepcopy(time_context[:,curr_step:curr_step+data_in.num_steps+2]), 
                                                    graph_dyn_edges=deepcopy(graph_dyn_edges[:,curr_step+1:curr_step+data_in.num_steps+2,:,:]), 
                                                    graph_masks_old = graph_masks, 
                                                    activity_masks_old = activity_masks,
                    ...
                    # if EXTRACAREFUL:
                    #     if (relocations_probs_wo_query[:,curr_step:1+curr_step,:,:].argmax(-1) != relocatn_probs_noquery.argmax(-1)).any():
                    #         prediction_change_at = torch.argwhere((relocations_probs_wo_query[:,curr_step:1+curr_step,:,:].argmax(-1) != relocatn_probs_noquery.argmax(-1)))[:,-1]
                    #         original_pred = relocations_probs_wo_query[0,curr_step:1+curr_step,prediction_change_at,:].argmax(-1)
                    #         new_pred = relocatn_probs_noquery[0,0,prediction_change_at,:].argmax(-1)
                    #         gt_pred = graph_seq_edges[:,curr_step+2,prediction_change_at,:].argmax(-1)
                    #         print(f"Previous query changed a prediction!! {prediction_change_at} from {original_pred} to {new_pred}, and GT is {gt_pred}.")
                    #         if (new_pred != gt_pred).any():
                    #             print(f" -- Wrong correction applied!!, Masks were {[o for o in obj_queries_buffer]}")
                    #             obj_wrong = prediction_change_at[torch.argwhere(new_pred != gt_pred)]
                    #             for o in obj_queries_buffer: 
                    #                 if obj_wrong == o['query']:
                    #                     print(f" ---- ###### ------ WRONG correction despite mask at!!, Masks were {obj_wrong} --> {original_pred[obj_wrong]} to {new_pred[obj_wrong]} : GT:{gt_pred[obj_wrong]}")
                    #                     print()
                    relocations_probs_wo_query[:,curr_step:1+curr_step,:,:] = deepcopy(relocatn_probs_noquery)
                    pred_activities_wo_query[:,curr_step:1+curr_step] = deepcopy(pred_activity_noquery[0].argmax(-1))
                    relocation_corrected[curr_step] = deepcopy(relocatn_probs_noquery).squeeze(0)
                    pred_activities_corrected[curr_step] = deepcopy(pred_activities_wo_query[0,curr_step:curr_step+1]).to(int)
            ...
            def evaluate_obj_query(mask, time_mask, ask_mask):
                ...
                if self.cfg.random_queries:
                    query_value = torch.randn_like(query_value).abs()
            ...
            if self.cfg.query_type in ['activity','both']:
                ...
                if activity_masks is None:
                    ...
                    # for actidx, act in enumerate(activities_checked):
                    #     if (oracle_labels == act).sum() > 0:
                ...
                # else:
                #     relocations_by_activity, _, pred_act_by_activity, changes_pred, _ = self.rollout_onestep_with_masks(
                #                                     latent_series=LatentDeterministic(deepcopy(latent_series.sample()), learn_magnitude=latent_series.learn_magnitude),
                #                                     time_context_series=deepcopy(time_context[:,:curr_step+1+data_in.num_steps,:]),
                #                                     num_steps=data_in.num_steps, 
                #                                     graph_seq_edges=deepcopy(graph_seq_edges[:,curr_step+1:curr_step+data_in.num_steps+2]),
                #                                     input_nodes_forward=deepcopy(graph_seq_nodes[:,curr_step+1:2+curr_step,:,:]), 
                ...
                #                                     reference_activity = activity_id_seq[:,curr_step:curr_step+1],
                #                                     activity_embedding_matrix=data_in.activity_embedding_matrix)
                #     value_local = torch.zeros((1,1)).to('cuda')
                #     relocation_correction[curr_step] = relocations_by_activity[0]
                #     pred_act_correction[curr_step] = pred_act_by_activity[0].argmax(-1)
                #     best_queries_act_viz[curr_step] = activity_masks.argmax(-1).squeeze(0)

                    if self.cfg.random_queries:
                        value_local = torch.randn_like(value_local).abs()
            ...
            if self.cfg.query_type in ['object','both']:
                ...
                # object_queries_list_strict = [torch.argwhere(cp).squeeze(-1) for cp in changes_at_step_wo_query[0,curr_step:curr_step+1,:]]
                rolled_over_obj_queries = [obj_q['query'][0] for obj_q in obj_queries_buffer]
                object_queries_list = [torch.tensor([o for o in object_queries_list_lenient_step if o not in rolled_over_obj_queries]).to('cuda') for object_queries_list_lenient_step in object_queries_list_lenient]
                ...
                if max(num_queries) > 0:
                    ...
                    queried_object = object_queries_list[0][best_queries_obj]
                    ...
                    # assert torch.allclose(((relocation_pos_obj*reference_edge_mask.squeeze(0))[torch.arange(len(best_queries_obj)), best_queries_obj, :]).sum(-1), torch.tensor([0.]).to(reference_edge_mask.device), atol=0.1), \
                    #     f"Masked to move objects are still predicted static!!\n Following should be 0\n{((relocation_pos_obj*reference_edge_mask.squeeze(0))[torch.arange(len(best_queries_obj)), best_queries_obj, :]).sum(-1)}"
                    # assert not EXTRACAREFUL or torch.allclose(((relocation_neg_obj*reference_edge_mask.squeeze(0))[torch.arange(len(best_queries_obj)), best_queries_obj, :]).sum(-1), torch.tensor([1.]).to(reference_edge_mask.device), atol=0.1), \
                    #     f"Masked to stay objects are still predicted to move!!\n Following should be 1\n{((relocation_neg_obj*reference_edge_mask.squeeze(0))[torch.arange(len(best_queries_obj)), best_queries_obj, :]).sum(-1)}"
            ...
            ## Carry forward previously resolved clarifications
            if not self.cfg.no_query_carryover:
                if act_correction:
                    act_queries_buffer = {'query': best_queries_act_viz[curr_step], 'steps_valid': data_in.num_steps-1}
                elif obj_correction:
                    obj_queries_buffer.append({'query': (best_queries_obj_viz[curr_step], oracle_positive_obj_at_step), 'steps_valid': data_in.num_steps-1})
            ...
            if obj_correction:
                assert not oracle_positive_obj_at_step or (relocation_corrected[curr_step].argmax(-1)[0,queried_object] != graph_seq_edges[0,1+curr_step:+2+curr_step,:,:].argmax(-1)[0,queried_object])
                assert oracle_positive_obj_at_step or (relocation_corrected[curr_step].argmax(-1)[0,queried_object] == graph_seq_edges[0,1+curr_step:+2+curr_step,:,:].argmax(-1)[0,queried_object])
        ...
        assert not self.cfg.no_query_carryover or ((num_carried_act_queries == 0) and (num_carried_obj_queries == 0))
        ...
        # self.results_with_clarification[self.cfg.query_type]['queries_spread'] += correction_masks
        ...
        assert (obj_mask[0,0,:] == obj_mask[0,5,:]).all(), "Just checking!!"
        obj_mask_single_dim = obj_mask[0,0,:]
        dense_gt = data_in.dest_gt.permute(2,0,1)[obj_mask_single_dim].permute(1,2,0).squeeze(0)
        snapshot_gt = deepcopy(data_in.dest_gt)
        snapshot_gt[torch.bitwise_not(data_in.changes_gt)] = -1
        snapshot_gt = snapshot_gt.permute(2,0,1)[obj_mask_single_dim].permute(1,2,0).squeeze(0)
        x=0.1
        snapshot_gt_alpha = ((((relocation_corrected*F.one_hot(data_in.dest_gt)).sum(-1))*((1-x)/.5))+x).clip(0,1)
        snapshot_gt_alpha = snapshot_gt_alpha.permute(2,0,1)[obj_mask_single_dim].permute(1,2,0).squeeze(0)
        dense_pred = deepcopy(dest_pred.permute(2,0,1)[obj_mask_single_dim].permute(1,2,0).squeeze(0))
        snapshot_pred = deepcopy(dest_pred)
        snapshot_pred[torch.bitwise_not(changes_pred)] = -1
        snapshot_pred = snapshot_pred.squeeze(0).permute(1,0)[obj_mask_single_dim].permute(1,0)
        pred_alpha = (data_in.changes_pred != changes_pred).squeeze(0).permute(1,0)[obj_mask_single_dim].permute(1,0).to(int)*0.5+0.5
        assert graph_seq_nodes.size(0) == 1, "This hack doesn't work with >1 batch size"
        node_idxs = (data_in.batch.get('node_ids')[0,0,0,:][obj_mask_single_dim]).to(int).reshape(-1)
        potential_movements = relocation_corrected.squeeze(0).clone().detach()
        potential_movements[F.one_hot(graph_seq_edges[0,1:pred_seq_len+1,:,:].argmax(-1), num_classes = relocation_corrected.size()[-1]).to(bool).squeeze(0)] = 0.0
        potential_movements_loc = potential_movements.argmax(-1).permute(1,0)[obj_mask_single_dim].permute(1,0)
        potential_movements_alpha = ((potential_movements.max(-1)[0]*((1-x)/.5))+x).clip(0,1).permute(1,0)[obj_mask_single_dim].permute(1,0).squeeze(0)
        
        if self.cfg.query_type not in self.snapshots_queries: self.snapshots_queries[self.cfg.query_type] = []

        value_act_alpha = deepcopy(value_act)
        # latents_forward = Some mix of one_step_latents if we want latents corrected through feedback to affect future steps

        self.snapshots_queries[self.cfg.query_type].append(adict({'pred':adict({'dense':dense_pred, 'alpha':pred_alpha, 'snapshot':snapshot_pred, 'activities':pred_activities_corrected, 
                                                                    'snapshot_wo_query': data_in.snapshot_pred, 'activities_wo_query': data_in.pred_activities_all}), 
                                                    'gt':adict({'dense':dense_gt, 'snapshot':snapshot_gt, 'alpha':snapshot_gt_alpha, 'activities':data_in.expected_activities}), 
                                                    'obj_mask':obj_mask_single_dim, 'node_idxs':node_idxs, 
                                                    'probs':adict({'location':potential_movements_loc, 'alpha':potential_movements_alpha}),
                                                    'queries':adict({'obj_queries': F.one_hot(best_queries_obj_viz.detach(), num_classes=obj_mask.size()[-1]).permute(1,0)[obj_mask[0,0,:]].permute(1,0), 
                                                            'obj_queries_carryover': queries_obj_viz_carryover.permute(1,0)[obj_mask[0,0,:]].permute(1,0), 
                                                            'obj_value': value_obj, 
                                                            'obj_oracle': oracle_positive_obj, 
                                                            'act_queries': best_queries_act_viz, 
                                                            'act_queries_carryover': queries_act_viz_carryover, 
                                                            'act_value': value_act_alpha, 
                                                            'act_oracle': best_queries_act_viz==pred_activities_wo_query[0,:]})}))
    ...
    def training_step(self, batch, batch_idx):
        ...
        # assert not torch.any(torch.Tensor([torch.isnan(v) for v in results['loss'].values()])), results['loss']
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # recall = self.results['moved']['correct'][0]/(self.results['moved']['total'][0]+1e-8)
        # precision = self.results['precision']['correct'][0]/(self.results['precision']['total'][0]+1e-8) 
        # f1_score_zerostep = 2*precision*recall/(precision+recall+1e-8)
        # print(f'\nValidation F1 Score : {f1_score_zerostep}')
    ...
    def write_results(self, output_dir, common_data, suffix=''):
        ...
        torch.save({'activity_consistencies':std_for_activity, 
                    ...
                    'max_leniency': self.cfg.leniency+1,
                    'data':self.result_data}, os.path.join(output_dir,f'raw_results_{suffix}.pt'))
        for leniency in range(self.cfg.leniency+1):
            json.dump(get_metrics(self.result_data, leniency=leniency, node_classes=node_classes_in_order, activity_consistencies=std_for_activity, obj_time_inconsistency=obj_time_inconsistency_masks), open(os.path.join(output_dir,f'test_evaluation_post_{suffix}_{leniency}leniency_all.json'),'w'), indent=4)
            json.dump(get_metrics(self.result_data, leniency=leniency, node_classes=node_classes_in_order, activity_consistencies=std_for_activity, obj_time_inconsistency=obj_time_inconsistency_masks, split_objects_by='taking_out'), open(os.path.join(output_dir,f'test_evaluation_post_{suffix}_{leniency}leniency_taking_out.json'),'w'), indent=4)
            json.dump(get_metrics(self.result_data, leniency=leniency, node_classes=node_classes_in_order, activity_consistencies=std_for_activity, obj_time_inconsistency=obj_time_inconsistency_masks, split_objects_by='putting_away'), open(os.path.join(output_dir,f'test_evaluation_post_{suffix}_{leniency}leniency_putting_away.json'),'w'), indent=4)
        if self.cfg.query_opportunity == 'last':
            for qt in self.results_with_clarification.keys():
                ...
                # self.results_with_clarification[qt]['queries_spread'] = self.results_with_clarification[qt]['queries_spread'].view(-1).tolist()
