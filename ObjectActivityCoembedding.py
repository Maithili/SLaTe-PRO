...
GraphDecoder = {
    'explicit': GraphDecoderExplicitModule,
    # 'transfGNN': GraphDecoderTransformerGNNModule,
    'simple': GraphDecoderModule
}
...
class LatentVariational(Latent):
    def __init__(self, data=None, mu=None, var=None):
        """
        data: [mu \in R^n, variance-nonpositive \in R^n]
        """
        super().__init__()
        assert (data is None) or (mu is None and var is None), f"Provide only one of data, mu and var: {data}, {mu}, {var}"
        if data is not None:
            min_std = 1e-2
            assert data.size(-1)%2 == 0, "latent size must be even"
            ## REMOVE THIS HACK!!!
            if len(data.size()) == 3: # "latent must be of shape 3D: batch_size x sequence_length x latent_size"
                self.mu = data[:,:,:data.size(-1)//2]
                self.var = torch.log(torch.exp(data[:,:,data.size(-1)//2:]) + 1) + min_std   # softplus
            elif len(data.size()) == 2: # "latent must be of shape 3D: batch_size x sequence_length x latent_size"
                self.mu = data[:,:data.size(-1)//2]
                self.var = torch.log(torch.exp(data[:,data.size(-1)//2:]) + 1) + min_std   # softplus
            else:
                raise Exception("latent must be 2D or 3D")
        elif mu is not None and var is not None:
            self.mu = mu
            self.var = var
        else:
            raise Exception(f"Provide something!! {data}, {mu}, {var}")
        self.eps = torch.randn_like(self.mu).to('cuda').detach()
        self._sample = self.mu + self.eps*torch.pow(self.var, 0.5)
    
    def __getitem__(self, index):
        mu = self.mu[index]
        var = self.var[index]
        return LatentVariational(mu=mu, var=var)

    def sample(self):
        return self._sample

    def __add__(self, second_latent):
        if isinstance(second_latent, LatentVariational):
            mu = (self.mu*second_latent.var + second_latent.mu*self.var)/(self.var + second_latent.var)
            var = torch.pow((torch.pow(second_latent.var, -1)+torch.pow(self.var, -1)), -1)
            return LatentVariational(mu=mu, var=var)
        if isinstance(second_latent, torch.Tensor):
            assert second_latent.size() == self.mu.size(), "latent size must be the same as mu size"
            mu = self.mu + second_latent
            return LatentVariational(mu=mu, var=self.var)
        else:
            raise ArgumentError("second_latent for LatentVariational must be of type LatentVariational or tensor")

    def _get_dist(self, mask=True):
        mu = self.mu[mask]
        var = self.var[mask]
        return torch.distributions.multivariate_normal.MultivariateNormal(mu, torch.diag_embed(torch.pow(var, 0.5)))
    ...
    def difference(self, latent_dist2=None, mask=True):
        ...
        mu1 = self.mu[mask]
        var1 = self.var[mask]
        if latent_dist2 is None:
            mu2 = torch.zeros_like(mu1)
            var2 = torch.zeros_like(var1)
        else:
            assert isinstance(latent_dist2, LatentVariational)
            mu2 = latent_dist2.mu[mask]
            var2 = latent_dist2.var[mask]
        if latent_dist2 is None:
            latent_dist2 = LatentVariational(mu=torch.zeros_like(self.mu), var=torch.ones_like(self.var))
        return kl_divergence_analytical(mu1=mu1, var1=var1, mu2=mu2, var2=var2)
        
    def append(self, latent2, dim=1):
        assert isinstance(latent2, LatentVariational)
        self.mu = torch.cat([self.mu, latent2.mu.unsqueeze(dim)], dim=dim)
        self.var = torch.cat([self.var, latent2.var.unsqueeze(dim)], dim=dim)
        self._sample = self.mu + self.eps*torch.pow(self.var, 0.5)

    def unsqueeze(self, dim):
        for t in [self.mu, self.var, self._sample]:
            t = t.unsqueeze(dim)
        return self
...
class ObjectActivityCoembeddingModule(LightningModule):
    def __init__(self, model_configs, original_model=None):
        ...
        self.latent_obj = LatentVariational if self.cfg.variational else LatentDeterministic

        ### Object AutoEncoder ###
        graph_encoder_module_local = GraphEncoder[self.cfg.encoder_type](model_configs=model_configs)
        ...
        graph_decoder_module = GraphDecoder[self.cfg.decoder_type](model_configs=model_configs)
        self.obj_seq_decoder = graph_decoder_module

        ### Activity AutoEncoder ###
        self.individual_embedding_size = int(self.embedding_size*2) if self.cfg.variational else self.embedding_size
    ...
    def latent_loss(self, latent_obj, latent_act, mask=True, allow_regularization=True):
        ...
        # if isinstance(latent_obj, LatentVariational) and isinstance(latent_act, LatentVariational): 
        #     latent_loss = latent_obj.difference(latent_act, mask=mask)
        #     latent_loss += latent_obj.difference(mask=mask)
        #     latent_loss += latent_act.difference(mask=mask)
            
        # elif isinstance(latent_obj, LatentDeterministic) and isinstance(latent_act, LatentDeterministic):
        #     if mask is not None:
        #         if mask.sum() == 0:
        #             return 0
        #         latent_obj = latent_obj[mask].view(-1, self.embedding_size)
        #         latent_act = latent_act[mask].view(-1, self.embedding_size)
        #     else:
        #         latent_obj = latent_obj.view(-1, self.embedding_size)
        #         latent_act = latent_act.view(-1, self.embedding_size)

        #     assert len(latent_obj.size()) == 2, f"Wrong size of latents {latent_obj.size()}, {latent_act.size()}, when mask is {mask.size()}"
        #     assert latent_obj.size()[1] == self.embedding_size
        #     flat_batch_size = latent_obj.size()[0]
        #     latent_loss = self.context_loss(F.normalize(latent_obj, dim=-1), F.normalize(latent_act, dim=-1))
    ...
    def decode_graph(self, latents, input_nodes, input_edges, dynamic_edges_mask, output_edges=None, activity_relevant_edges=None, activity_mask=None):
        ...
        if output_edges is not None:
            ...
            if self.cfg.funky_graph_loss:
                graph_pred_loss = self.obj_graph_loss_conf_matrix(pred_edges, output_edges, ref=input_edges.view(batch_size*(sequence_len), self.cfg.n_nodes, self.cfg.n_nodes), obj_mask=obj_mask)
            else:
